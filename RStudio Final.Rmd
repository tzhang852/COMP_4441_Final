...
```{r}
t.test(clean_dat$mearn,clean_dat$wearn)
```

```{r}
t.test(clean_dat$mearn,clean_dat$wearn, paired = TRUE, conf.level = .99)
```

```{r}
library(pwr)
pwr.t.test(n=596, d = 0.8, sig.level=0.99, power = NULL, type = c("paired"), alternative = c("greater"))
```


```{r}
salary.lm=lm(mearn ~ wearn, data = clean_dat)

summary(salary.lm)

plot(salary.lm, which=1)
plot(salary.lm, which=2)
```